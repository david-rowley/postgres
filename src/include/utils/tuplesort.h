...
/*
 * The following two struct definitions are what we actually perform the sort
 * upon.  We most commonly use SortTuple.  These are what we normally pass to
 * our sort implementation.  SortTuple store both the tuple and also a Datum
 * value.  The Datum value duplicates the first sort column's value for
 * by-val column types.  This allows the value of the first column to be
 * accessed without having to repeatedly deform the tuple each time we must
 * compare two tuples.  We'll only need to perform tuple deformation when the
 * comparison finds the first columns are equal.  We could provide storage for
 * more than 1 column, but it's likely that the majority of comparisons won't
 * require comparison of the 2nd column.  If we were to provide space for
 * further columns, it's likely to cause slowdowns from the additional memory
 * requirements.
 *
 * For tuples where the first column value is NULL, we store these in a
 * separate array in Tuplesortstate.  We will sort the 'nulltuples' array by
 * any remaining sort columns  at the same time we we sort the 'memtuples'.
 * The array which we output first depends on NULLS FIRST / NULLS LAST.
 *
 * The tuples pointed to by SortTuple and the tuples stored in Tuplesortstate
 * 'nulltuples' array might be a MinimalTuple or IndexTuple, which is a
 * separate palloc'd chunk --- we assume it is just one chunk and can be freed
 * by a simple pfree() (except during merge, when we use a simple slab
 * allocator and when performing a non-bounded sort where we use a bump
 * allocator).
 *
 * When we're unable to perform the entire sort in memory, we perform the sort
 * in batches.  When Tuplesortstate.availMem is too low to store another
 * tuple, we perform the sort, then write out those tuples to tape.  Multiple
 * tapes are compared while outputting the tuples by using a k-way merge
 * algorithm.  When we read the tuples back from take, we store these in a
 * DiskTuple.  This is done as we need to record which tape index the given
 * tuple came from.
 */
typedef struct
{
