...
add_eq_member(PlannerInfo *root, EquivalenceClass *ec, Expr *expr,
			  Relids relids, JoinDomain *jdomain, EquivalenceMember *parent,
			  Oid datatype)
{
						 ...
						 bool create_it)
{
	...
	expr_relids = pull_varnos_without_nullingrels(root, (Node *) expr);
							 ...
							 bool outer_on_left)
{
	...
	foreach(lc1, root->eq_classes)
	{
		...
		while ((cur_em = eclass_member_iterator_strict_next(&iter)) != NULL)
		{
			Assert(!cur_em->em_is_child);	/* no children expected */
