...
tts_virtual_copyslot(TupleTableSlot *dstslot, TupleTableSlot *srcslot)
{
	...
	{
		CompactAttribute *catt = TupleDescCompactAttr(dstdesc, natt);
		Datum		val;
		bool		isnull;
		...
		{
			/*
			 * when dstslot has more natts than srcslot we must materialize
			...
			VARATT_IS_EXTERNAL_EXPANDED(DatumGetPointer(val)))
		{
			 ...
			 * slot doesn't depend on it.
			 */
			sz = att_nominal_alignby(sz, catt->attalignby);
			sz += EOH_get_flat_size(DatumGetEOHP(val));
		}
		else
		{
			sz = att_nominal_alignby(sz, catt->attalignby);
			sz = att_addlength_datum(sz, catt->attlen, val);
		}
	}
	...
	if (sz > 0)
	{
		...
		for (int natt = 0; natt < dstdesc->natts; natt++)
		{
			...
			if (catt->attbyval || dstslot->tts_isnull[natt])
				continue;
				...
				VARATT_IS_EXTERNAL_EXPANDED(DatumGetPointer(val)))
			{
				...
				data = (char *) att_nominal_alignby(data, catt->attalignby);
				data_length = EOH_get_flat_size(eoh);
				EOH_flatten_into(eoh, data, data_length);
			...
			else
			{
				...
				data = (char *) att_nominal_alignby(data, catt->attalignby);
				data_length = att_addlength_datum(data_length, catt->attlen,
												  val);
