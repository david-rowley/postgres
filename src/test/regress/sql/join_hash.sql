...
begin
  ...
  loop
    ...
    original := greatest(hash_node->>'Original Hash Batches',
                         (select max(a->>'Original Hash Batches') from json_array_elements(hash_node->'Workers') a));
    final := greatest(hash_node->>'Hash Batches',
                      (select max(a->>'Hash Batches') from json_array_elements(hash_node->'Workers') a));
    return next;
  end loop;
end;
$$;
